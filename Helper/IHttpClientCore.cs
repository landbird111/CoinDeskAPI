...
public interface IHttpClientCore
{
    ...
    /// <returns>回應訊息。</returns>
    Task<HttpResponseMessage> PostAsync(Uri requestUri, HttpContent content, TimeSpan timeout);
    ...
    /// <param name="timeout">請求的超時時間。</param>
    /// <returns>回應訊息。</returns>
    Task<HttpResponseMessage> GetAsync(Uri requestUri, TimeSpan timeout);
}
